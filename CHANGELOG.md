...
- Peer routing documents what HTTPS to a forwarded hostname needs: a certificate on the machine doing the reaching, because TLS terminates there. `tailscale-peers` names `generate-mkcert` when anything is forwarded, and the README shows the failure, the command, and why a `*.spark.loc` wildcard does not cover `macos.test.spark.loc` ([#118](https://github.com/sparkfabrik/http-proxy/issues/118))
