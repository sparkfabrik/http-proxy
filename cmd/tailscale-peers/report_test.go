...
func TestRenderPointsAtCertificatesWhenSomethingIsForwarded(t *testing.T) {
	forwarding := &Report{
		UpdatedAt: time.Now().UTC(),
		Peers: []PeerReport{
			{Name: "desktop", Address: "100.64.0.1", Status: statusOK, Hosts: []string{"app.loc"}},
...
func TestRenderSaysNothingAboutCertificatesWhenNothingIsForwarded(t *testing.T) {
	// The line is about hostnames reached over HTTPS. With none forwarded there
	// is nothing to reach, and the advice would be noise on every cycle.
	quiet := &Report{
		UpdatedAt: time.Now().UTC(),
		Peers: []PeerReport{
			{Name: "phone", Address: "100.64.0.2", Status: statusSkipped, Reason: "offline"},
	...
	if out := quiet.Render(); strings.Contains(out, "generate-mkcert") {
		t.Errorf("a report with nothing forwarded named the certificate command anyway:\n%s", out)
	}
}
